...
def cyclize_linpep_from_smi(smi, link):
    ...
    link: the link between the two ends, e.g. '4:R3-10:R2'
    """
    smi = relabel_rgroup2index(smi)
    link = link.split('-')
    start, end = (int(end.split(':')[1][1:]) for end in link)
...
def restore_unused_rgroup(monomer_smis, monomer_r_groups, monomer_links):
    ...
    for idx, mol_cxsmi in enumerate(monomer_smis):
        ...
        used_r_groups = [r_group[1:]
                         for r_group in list(monomer_links[idx].keys())]
...
def cyclize_linpep_from_helm(linear_helm, cyclic_link):
    ...
    monomer_smis, monomer_r_groups = [], []
    for monomer in monomer_list:
        if monomer in monomers2smi_dict:
            monomer_smi = copy.deepcopy(monomers2smi_dict[monomer])
            monomer_r_group = copy.deepcopy(monomers2r_groups_dict[monomer])

            monomer_smis.append(monomer_smi)
            monomer_r_groups.append(monomer_r_group) # Default R groups if no linker
        else:
            logger.warning(f'Unknown monomer {monomer}')
