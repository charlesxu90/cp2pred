...
class GradCAM():
    ...
    def __call__(self, img, class_index=None):
        assert img[1].shape[0] == 1
        img_show, img = img[0], img[1]
        input = img.reshape(-1, 3, 224, 224).cuda()
...
def get_gradcam_plus_img(mask, img, thresh=.0):
...
def get_gradcam(gradcam_obj, img_path):
    img_cuda, img_show = get_transformed_img(img_path)
    gradcam = gradcam_obj(img=(img_show, img_cuda))
    img_gradcam, _ = get_gradcam_plus_img(gradcam, img_show, thresh=.0)
...
def run_gradcam(img_path, model):
    gradcam_obj = GradCAM(model=model, thresh=0.0)
    img_gradcam, img_show = get_gradcam(gradcam_obj, img_path)
